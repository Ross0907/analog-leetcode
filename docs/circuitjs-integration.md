...
The application is self-hosted under `public/circuitjs/`, framed from the same origin, and works without loading a third-party hosted editor. The native `CircuitJS1` JavaScript API provides timestep callbacks, node voltages, supported branch currents and native circuit serialization. The small GPL patch exposes native terminal coordinates, node IDs, hit testing, element XML attributes, stop messages and viewport projection for probe placement and existing-grader interoperability. It also removes a redundant post-drag undo checkpoint so one native Undo reverses one edit. It changes no electrical, routing, symbol or rendering algorithms.
